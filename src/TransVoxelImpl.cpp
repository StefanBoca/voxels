...
namespace Voxels
{
    ...
    struct TransVoxelRun
    {
        ...
        bool AreBlockAndNeighborsEmpty(Block& block) const
        {
            ...
            for (int z = -1; z < 2; ++z)
                for (int y = -1; y < 2; ++y)
                    for (int x = -1; x < 2; ++x)
                    {
                        glm::vec3 coord = glm::vec3(std::clamp(block.Coords.x + x, 0.f, m_BlockCounts[0].x - 1),
                                std::clamp(block.Coords.y + y, 0.f, m_BlockCounts[0].y - 1),
                                std::clamp(block.Coords.z + z, 0.f, m_BlockCounts[0].z - 1));
        ...
        typedef std::unordered_map<int, GridBlocksCache*> GridBlockMap;
